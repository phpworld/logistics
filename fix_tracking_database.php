...
/**
 * Fix Tracking Database Script
 * 
 * This script fixes the duplicate key issue by:
 * 1. Dropping existing tracking tables
 * 2. Re-running migrations
 * 3. Creating fresh sample data
 */
...
echo "🔧 Fixing Royal Mail Tracking Database...\n\n";

$db = \Config\Database::connect();

// Drop existing tables if they exist
echo "🗑️  Dropping existing tracking tables...\n";

try {
    // Drop tables in correct order (child tables first)
    $db->query('SET FOREIGN_KEY_CHECKS = 0');
    
    if ($db->tableExists('tracking_timeline')) {
        $db->query('DROP TABLE tracking_timeline');
        echo "✅ Dropped tracking_timeline table\n";
    }
    
    if ($db->tableExists('tracking_parcels')) {
        $db->query('DROP TABLE tracking_parcels');
        echo "✅ Dropped tracking_parcels table\n";
    }
    
    $db->query('SET FOREIGN_KEY_CHECKS = 1');
    
} catch (Exception $e) {
    echo "⚠️  Warning during table drop: " . $e->getMessage() . "\n";
}

// Clear migration history for tracking tables
echo "\n🧹 Clearing migration history...\n";
try {
    $db->table('migrations')->where('filename', '2025-01-16-120000_CreateTrackingParcelsTable')->delete();
    $db->table('migrations')->where('filename', '2025-01-16-120001_CreateTrackingTimelineTable')->delete();
...
// Check tracking_parcels table
if ($db->tableExists('tracking_parcels')) {
    echo "✅ tracking_parcels table exists\n";
    
    // Check for unique key on tracking_number
    $keys = $db->query("SHOW KEYS FROM tracking_parcels WHERE Column_name = 'tracking_number'")->getResultArray();
    $hasUniqueKey = false;
    foreach ($keys as $key) {
        if ($key['Non_unique'] == 0) {
            $hasUniqueKey = true;
    ...
    if ($hasUniqueKey) {
        echo "✅ Unique key on tracking_number exists\n";
    } else {
        echo "❌ Unique key on tracking_number missing\n";
    }
} else {
    echo "❌ tracking_parcels table missing\n";
    exit(1);
}

// Check tracking_timeline table
if ($db->tableExists('tracking_timeline')) {
    echo "✅ tracking_timeline table exists\n";
} else {
    echo "❌ tracking_timeline table missing\n";
    exit(1);
}
...
foreach ($sampleData as $data) {
    try {
        ...
        echo "❌ Failed to create tracking record " . $data['tracking_number'] . ": " . $e->getMessage() . "\n";
    }
}
...
echo "3. Test tracking management\n\n";
        ...
        case 'delivered':
            $events = [
                [
                    ...
                    'event_date' => date('Y-m-d', $currentTime - 259200),
                    'event_time' => '11:15:00',
                    'icon' => 'fas fa-cog',
                    'color' => 'secondary',
                    'created_at' => date('Y-m-d H:i:s')
